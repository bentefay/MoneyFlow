...
const PasswordDescription = ({ password }: { password: string | null }) => {
    if (password === null || password.length == 0)
        return (
            <>
                Must be <b>12 or more</b> letters, numbers or symbols
            </>
        );
    ...
    if (remainingCharactersRequired > 1)
        return (
            <>
                Just <b>{remainingCharactersRequired}</b> more letters, numbers or symbols
            </>
        );
    ...
    if (remainingCharactersRequired == 1)
        return (
            <>
                Just <b>1</b> more letter, number or symbol!
            </>
        );
...
const form = ({ createAccount, generalFailure, isLoading, onCreateAccountToggled, onSignInInitiated }: Props & Actions) => {
    ...
    return (
        <form
            ...
            <label htmlFor={passwordId} className={c.label}>
                Password
            </label>
            ...
            <div className={c.buttonRow}>
                <a
                    ...
                    {createAccount ? <>Sign in to account</> : <>Create account</>}
                </a>
